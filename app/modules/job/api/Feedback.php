<?php
namespace app\modules\job\api;

use Yii;
use app\modules\job\models\Feedback as FeedbackModel;

use yii\helpers\Html;
use yii\helpers\Url;
use yii\widgets\ActiveForm;
use yii\easyii\widgets\ReCaptcha;


/**
 * Feedback module API
 * @package app\modules\job\api
 *
 * @method static string form(array $options = []) Returns fully worked standalone html form.
 * @method static array save(array $attributes) If you using your own form, this function will be useful for manual saving feedback's.
 */

class Feedback extends \yii\easyii\components\API
{
    const SENT_VAR = 'feedback_sent';

    private $_defaultFormOptions = [
        'errorUrl' => '',
        'successUrl' => ''
    ];

    public function api_form($options = [])
    {
        $model = new FeedbackModel;
        $settings = Yii::$app->getModule('admin')->activeModules['job']->settings;
        $options = array_merge($this->_defaultFormOptions, $options);

        ob_start();
        $form = ActiveForm::begin([
            'enableClientValidation' => true,
            'action' => Url::to(['/job/send'])
        ]);

        echo Html::hiddenInput('errorUrl', $options['errorUrl'] ? $options['errorUrl'] : Url::current([self::SENT_VAR => 0]));
        echo Html::hiddenInput('successUrl', $options['successUrl'] ? $options['successUrl'] : Url::current([self::SENT_VAR => 1]));

        echo $form->field($model, 'name')->textInput(['placeholder' => 'Namn *'])->label(false);
        if($settings['enablePhone']) echo $form->field($model, 'phone')->textInput(['placeholder' => 'Telefon *'])->label(false);
        echo $form->field($model, 'email')->input('email',['placeholder' => 'E-post *'])->label(false);
        if($settings['enableTitle']) echo $form->field($model, 'title')->textInput(['placeholder' => 'Erfarenhet som tolk (antal år) *'])->label(false);

        echo $form->field($model, 'language')->textInput(['placeholder' => 'A- Språk (modersmål) *'])->label(false);
        echo $form->field($model, 'language2')->textInput(['placeholder' => 'B- Språk'])->label(false);
        echo $form->field($model, 'language3')->textInput(['placeholder' => 'C- Språk'])->label(false);
        echo $form->field($model, 'education')->textInput(['placeholder' => 'Har du genomgått en tolkutbildning? *'])->label(false);

        echo $form->field($model, 'text')->textarea(['value' => 'Skriv ditt meddelande här...'])->label(false);

        if($settings['enableCaptcha']) echo $form->field($model, 'reCaptcha')->widget(ReCaptcha::className());

        echo Html::submitButton(Yii::t('easyii', 'Send'), ['class' => 'btn btn-success']);
        ActiveForm::end();

        return ob_get_clean();
    }

    public function api_save($data)
    {
        $model = new FeedbackModel($data);
        if($model->save()){
            return ['result' => 'success'];
        } else {
            return ['result' => 'error', 'error' => $model->getErrors()];
        }
    }
}