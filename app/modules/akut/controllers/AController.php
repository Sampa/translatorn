...
class AController extends Controller
{
    ...
    public function init()
    {
        ...
        $this->new = Yii::$app->getModule('admin')->activeModules['akut']->notice;
        $this->noAnswer = Akut::find()->status(Akut::STATUS_VIEW)->count();
    }

    public function actionIndex()
    {
        $data = new ActiveDataProvider([
            'query' => Akut::find()->status(Akut::STATUS_NEW)->asc(),
        ]);
    ...
    public function actionNoanswer()
    {
        ...
        $data = new ActiveDataProvider([
            'query' => Akut::find()->status(Akut::STATUS_VIEW)->asc(),
        ]);
        return $this->render('index', [
            'data' => $data
        ]);
